...
@router.get("/{id_loan}/books", tags=["Loan Books"], status_code=status.HTTP_200_OK)
async def get_all_books_of_loan( id_loan: int ):
    result = await get_all_books(id_loan)
    return result

@router.get("/{id_loan}/books/{id_book}", tags=["Loan Books"], status_code=status.HTTP_200_OK)
async def get_one_book_of_loan( id_loan: int, id_book: int ):
    result = await get_one_book(id_loan, id_book)
    return result

@router.post("/{id_loan}/books", tags=["Loan Books"], status_code=status.HTTP_201_CREATED)
async def assign_book_to_loan( id_loan: int, book_data: Loan_Books ):
    result = await add_book_to_loan(id_loan, book_data.id_book)
    return result

@router.put("/{id_loan}/books/{id_book}", tags=["Loan Books"], status_code=status.HTTP_201_CREATED)
...
@router.delete("/{id_loan}/books/{id_book}", tags=["Loan Books"], status_code=status.HTTP_204_NO_CONTENT)
