...
@router.get("/{id_author}/books", tags=["Authors Books"], status_code=status.HTTP_200_OK)
async def get_all_books_of_author( id_author: int ):
    result = await get_all_books(id_author)
    return result

@router.get("/{id_author}/books/{id_book}", tags=["Authors Books"], status_code=status.HTTP_200_OK)
async def get_one_book_of_author( id_author: int, id_book: int ):
    result = await get_one_book(id_author, id_book)
    return result

@router.post("/{id_author}/books", tags=["Authors Books"], status_code=status.HTTP_201_CREATED)
...
@router.delete("/{id_author}/books/{id_book}", tags=["Authors Books"], status_code=status.HTTP_204_NO_CONTENT)
